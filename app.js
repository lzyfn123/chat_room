    ...
    socket.on('online', function (data) {
        ...
        if (!clients[data.user]) {
            ...
            for (var index in clients) {
                clients[index].emit('system', JSON.stringify({type: 'online', msg: data.user, time: getCurrentTime()}));
                clients[index].emit('userflush', JSON.stringify({users: users}));
            }
            socket.emit('system', JSON.stringify({type: 'in', msg: '', time: getCurrentTime()}));
